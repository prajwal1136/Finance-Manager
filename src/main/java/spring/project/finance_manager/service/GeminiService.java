...
//    @Value("${AIzaSyDCNONTAZKdhlBLQyaKGnW5Zyn7RNsrIa4}")
...
public class GeminiService {
    ...
    public ResponseEntity<?> chatWithBot(String accessToken, String refreshToken,
        ...
        try {
            ...
            // If you want AI, wrap in try-catch to prevent failures
            try {
                // AI categorization or response code here (optional)
            } catch (Exception e) {
                System.out.println("Gemini API failed: " + e.getMessage());
            }

            return ResponseEntity.ok("AI temporarily disabled.");
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Error: " + e.getMessage());
        }
    }
    ...
    public String categorizeTransaction(String description) {
        // TEMPORARY bypass: return placeholder category
        return "Uncategorized";
    }
}
